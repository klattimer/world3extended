...
@njit(cache=True)
def _pollution_update(cumulative: float, emissions: float, sink_rate: float) -> float:
    return cumulative + emissions - sink_rate * cumulative
...
class ClimateSystem:
    ...
    def step(self, state: dict[str, float]) -> dict[str, float]:
        ...
        cumulative_pollution = float(_pollution_update(state["cumulative_pollution"], emissions, sink_rate))
        pollution_index = max(0.0, cumulative_pollution / 100.0)

        warming = state["warming_c"] + warming_sensitivity * np.log1p(max(0.0, pollution_index - 0.8))
        ...
        migration_pressure = float(
            np.clip(state["migration_pressure"] * 0.9 + 0.25 * disaster_pressure + 0.15 * state["food_stress"], 0.0, 1.0)
        )
